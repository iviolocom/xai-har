...
#!/usr/bin/env python3
# python inference_each_set_video_dashboard.py --cfg configs/Kinetics/X3D_L.yaml NUM_GPUS 1 TRAIN.ENABLE False DATA.PATH_TO_DATA_DIR datasets22
...
def activity_localization(prob_sq, action_threshold):
    ...
    for i in range(len(action_tag)):
        action_idxs.append(action_idx[i])
        if action_tag[i] ==1:
    ...
    print('end', endings)
    return action_idxs,activities_idx,startings,endings
...
if __name__ == "__main__":
    ...
    # print("path ", path)
    df = pd.DataFrame(columns=['vid_id', 'action_idxs','action_idxs_counter','action_idxs_len','activities_idx','activities_counter', 'activities_len','action_idxs_prob','activities_prob'])
    for k in range(16):
        video_ids={}
        video_names = []
        video_ids_file = os.path.join(path, 'video_ids_db' + str(k) +'.csv')
        print("video_ids_file ",video_ids_file)
        with open(video_ids_file) as csvfile:
            csvReader = csv.reader(csvfile, delimiter=',')
            for idx, row in enumerate(csvReader):
                if idx > 0:
                    video_ids[row[1]] = row[0]
                    video_names.append(row[1])
        # print("video_ids, video_names:",video_ids, video_names)
        # path = './data/A2/'
        import glob
        ...
        # print("filelist[vid_name] ",filelist)
        ...
        checkpoint_dashboard_list = ['checkpoint_submit/checkpoint_epoch_dashboard_10018_85.71_98.76.pyth',
                                    'checkpoint_submit/checkpoint_epoch_dashboard_20018_73.75_97.50.pyth',
                                    'checkpoint_submit/checkpoint_epoch_dashboard_30014_78.12_95.62.pyth',
                                    'checkpoint_submit/checkpoint_epoch_dashboard_40017_81.88_99.38.pyth',
                                    'checkpoint_submit/checkpoint_epoch_dashboard_50012_83.33_97.02.pyth']  
        vid_info = dict(sorted(vid_info.items()))
        prob_1, video_order = main(cfg, vid_info, labels, filelist, checkpoint_dashboard_list)

        # print("len(vid_info)",len(vid_info))
        ...
        for i in range(1, len(vid_info)+1):
            print(" vid_info: ", video_order[i-1])
            try:
                len_prob = len(prob_1[str(i)])
            except:
                print("Video _ info ", i)
                continue
            prob_ensemble_video = []
            for ids in range(len_prob):
                prob_sub_mean = prob_1[str(i)][ids]
                prob_ensemble_video.append(prob_sub_mean)
                prob_actions = np.array(prob_ensemble_video)
                # print("prob_actions 1",prob_actions)
                prob_actions = np.squeeze(prob_actions)
                # print("prob_actions 2",prob_actions)
            activity_threshold = 0.1
            action_idxs,activities_idx, startings, endings = activity_localization(prob_actions, activity_threshold)
            sum_activities += activities_idx
            sum_action_idxs += action_idxs
            # print("sum_activities: ",sum_activities)
        
        action_idxs_counter = sum_action_idxs.count(k)
        action_idxs_len = len(sum_action_idxs)
        action_idxs_prob = round(action_idxs_counter *100/action_idxs_len,2)
        print('Video action_idxs: ',video_ids_file,'; sum_action_idxs: ',sum_action_idxs,'; action_idxs_counter: ',action_idxs_counter,'; action_idxs_len: ',action_idxs_len,'; action_idxs_prob: ',action_idxs_prob)
        
        activities_counter = sum_activities.count(k)
        activities_len = len(sum_activities)
        activities_prob = round(activities_counter *100/activities_len,2)
        print('Video activities: ',video_ids_file,'; sum_activities: ',sum_activities,'; activities_counter: ',activities_counter,'; activities_len: ',activities_len,'; activities_prob: ',activities_prob)
        list_row = [k,action_idxs,action_idxs_counter,action_idxs_len,activities_idx,activities_counter, activities_len,action_idxs_prob,activities_prob]
        df.loc[len(df)] = list_row
    df.to_csv('logs/logdb0412.csv')    
            #     list = [video_ids_file,sum_activities]
    #     df_video_ids = df_video_ids.append(pd.Series(list, index = [0,1]), ignore_index=True)
    # df_video_ids.to_csv(r'./logs/logdb.csv', header=None, index=None, sep=', ', mode='w')
        # print("startings: ",startings)
        # print("endings: ",endings)
    
    # print("len(prob_1) ",len(prob_1))
    # print("prob_1",prob_1)
    # len_prob=prob_1['1']
    # print("len_prob",len_prob)

    # for ids in range(len_prob):
    #     print("len_prob[ids] ",len_prob[ids])
    # prob_ensemble = []
    # dataframe_list = []
    # #ensemble 3 model
    # #iterate each video
